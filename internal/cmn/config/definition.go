...
// OIDCRoleMappingDef maps OIDC claims to Ayatsuri roles.
type OIDCRoleMappingDef struct {
	...
	GroupMappings       map[string]string `mapstructure:"group_mappings"`        // IdP group -> Ayatsuri role
...
type TailscaleTunnelDef struct {
	...
	Hostname string `mapstructure:"hostname"`  // Default: "ayatsuri"
	Funnel   *bool  `mapstructure:"funnel"`    // Public internet access
	HTTPS    *bool  `mapstructure:"https"`     // HTTPS for tailnet-only access
	StateDir string `mapstructure:"state_dir"` // Default: $AYATSURI_HOME/tailscale
}
