...
type TailscaleTunnelConfig struct {
	...
	Hostname string // Machine name in tailnet (default: "ayatsuri")
	Funnel   bool   // Enable public internet access
	HTTPS    bool   // Enable HTTPS for tailnet-only access
	StateDir string // Default: $AYATSURI_HOME/tailscale
}
...
// OIDCRoleMapping defines how OIDC claims are mapped to Ayatsuri roles.
type OIDCRoleMapping struct {
	DefaultRole         string            // Default: "viewer"
	GroupsClaim         string            // Default: "groups"
	GroupMappings       map[string]string // IdP group -> Ayatsuri role
