...
`
	...
	t.Run("ScriptExecution", func(t *testing.T) {
		...
		// Note: Avoid shell variables with ${} as ayatsuri expands them before sending to SSH
		dagConfig := sshServer.sshConfig("/bin/sh") + `
steps:
  - name: script-test
    type: ssh
...
`
		...
		dag.Agent().RunSuccess(t)
		dag.AssertLatestStatus(t, core.Succeeded)
		// GREETING is DAG-scoped → expanded by Ayatsuri to "hello"
...
func startSSHServer(t *testing.T, th test.Helper, dockerClient *client.Client) *sshServerContainer {
	...
	containerName := fmt.Sprintf("ayatsuri-ssh-test-%d", time.Now().UnixNano())
...
func trySSHConnection(t *testing.T, addr string, config *ssh.ClientConfig) bool {
	...
	session.Stdin = strings.NewReader("__ayatsuri_exec(){\nset -e\necho test\n}\n__ayatsuri_exec\n")
