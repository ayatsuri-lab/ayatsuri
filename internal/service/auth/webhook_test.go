...
func TestService_CreateWebhook(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		assert.Contains(t, result.FullToken, "ayatsuri_wh_")
	})
...
func TestService_ValidateWebhookToken(t *testing.T) {
	...
	t.Run("InvalidTokenHash", func(t *testing.T) {
		...
		_, err = service.ValidateWebhookToken(ctx, "test-dag", "ayatsuri_wh_wrongtoken12345")
		assert.ErrorIs(t, err, ErrInvalidWebhookToken)
	})
	...
	t.Run("WebhookNotFound", func(t *testing.T) {
		...
		_, err := service.ValidateWebhookToken(ctx, "nonexistent-dag", "ayatsuri_wh_sometoken")
		assert.ErrorIs(t, err, ErrInvalidWebhookToken)
	})
	...
	t.Run("NoWebhookStore", func(t *testing.T) {
		...
		_, err := service.ValidateWebhookToken(ctx, "test-dag", "ayatsuri_wh_token")
		assert.ErrorIs(t, err, ErrWebhookNotConfigured)
	})
...
func TestGenerateWebhookToken(t *testing.T) {
	...
	t.Run("Format", func(t *testing.T) {
		...
		assert.True(t, len(parts.fullToken) > len("ayatsuri_wh_"))
		assert.Contains(t, parts.fullToken, "ayatsuri_wh_")
