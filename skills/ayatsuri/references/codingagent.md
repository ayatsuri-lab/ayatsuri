...
When Ayatsuri is launched from inside Claude Code, environment variables leak into child steps and cause `claude -p` to hang. **Unset these right before every `claude -p` call:**
