---
name: ayatsuri
description: Writes, validates, and debugs Ayatsuri DAG workflow definitions in YAML. Use when creating, editing, or troubleshooting Ayatsuri .yaml DAG files. Do not use for general YAML editing.
---
# Ayatsuri DAG Authoring

Use this skill for Ayatsuri DAG YAML work. Keep the main skill lean and load only the reference file that matches the task.
...
- `references/cli.md` when you need command flags or lookup commands such as `ayatsuri schema`, `ayatsuri config`, or `ayatsuri history`
- `references/env.md` when execution environment variables, `AYATSURI_*` config vars, or `params:`/`env:` resolution order matters
